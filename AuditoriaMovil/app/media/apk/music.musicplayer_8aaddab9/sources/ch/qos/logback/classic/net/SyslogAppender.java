...
public class SyslogAppender extends SyslogAppenderBase<ILoggingEvent> {
    ...
    private void handleThrowableFirstLine(OutputStream outputStream, IThrowableProxy iThrowableProxy, String str, boolean z) throws IOException {
        ...
        if (!z) {
            y.append(CoreConstants.CAUSED_BY);
        }
        y.append(iThrowableProxy.getClassName());
        y.append(": ");
        y.append(iThrowableProxy.getMessage());
        outputStream.write(y.toString().getBytes());
        outputStream.flush();
    }
    ...
    public String getPrefixPattern() {
        StringBuilder y = a.y("%syslogStart{");
        y.append(getFacility());
        y.append("}%nopex{}");
        return y.toString();
    }
