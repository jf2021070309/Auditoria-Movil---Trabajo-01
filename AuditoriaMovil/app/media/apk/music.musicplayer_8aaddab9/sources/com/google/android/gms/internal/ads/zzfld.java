...
public final class zzfld {
    ...
    public static PendingIntent zza(Context context, int i2, Intent intent, int i3, int i4) {
        boolean z = true;
        zzflx.zza((i3 & 95) == 0, "Cannot set any dangerous parts of intent to be mutable.");
        zzflx.zza(intent.getComponent() != null, "Must set component on Intent.");
        if (zzb(0, 1)) {
            zzflx.zza(!zzb(i3, 67108864), "Cannot set mutability flags if PendingIntent.FLAG_IMMUTABLE is set.");
        } else {
            if (Build.VERSION.SDK_INT >= 23 && !zzb(i3, 67108864)) {
                z = false;
            }
            zzflx.zza(z, "Must set PendingIntent.FLAG_IMMUTABLE for SDK >= 23 if no parts of intent are mutable.");
        }
