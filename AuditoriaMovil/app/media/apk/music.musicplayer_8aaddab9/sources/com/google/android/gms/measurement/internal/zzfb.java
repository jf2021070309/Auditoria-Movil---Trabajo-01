...
public final class zzfb implements Runnable {
    ...
    public final void run() {
        ...
        try {
            ...
            zzfdVar.zza.zzay().zzd().zzb("Exception occurred while retrieving the Install Referrer", e2.getMessage());
        }
        zzfdVar.zza.zzaz().zzg();
        zzfv.zzO();
        if (bundle2 != null) {
            ...
            if (j2 == 0) {
                a.H(zzfdVar.zza, "Service response is missing Install Referrer install timestamp");
            } else {
                String string = bundle2.getString("install_referrer");
                if (string == null || string.isEmpty()) {
                    a.F(zzfdVar.zza, "No referrer defined in Install Referrer response");
                } else {
                    zzfdVar.zza.zzay().zzj().zzb("InstallReferrer API result", string);
                    Bundle zzs = zzfdVar.zza.zzv().zzs(Uri.parse(string.length() != 0 ? CallerData.NA.concat(string) : new String(CallerData.NA)));
                    if (zzs == null) {
                        a.F(zzfdVar.zza, "No campaign params defined in Install Referrer result");
                    } else {
                        String string2 = zzs.getString("medium");
                        if (string2 != null && !"(not set)".equalsIgnoreCase(string2) && !"organic".equalsIgnoreCase(string2)) {
                            long j3 = bundle2.getLong("referrer_click_timestamp_seconds", 0L) * 1000;
                            if (j3 == 0) {
                                a.F(zzfdVar.zza, "Install Referrer is missing click timestamp for ad campaign");
                            } else {
                                zzs.putLong("click_timestamp", j3);
                            }
                        }
                        if (j2 == zzfdVar.zza.zzm().zzd.zza()) {
                            a.G(zzfdVar.zza, "Logging Install Referrer campaign from module while it may have already been logged.");
                        }
                        if (zzfdVar.zza.zzJ()) {
                            zzfdVar.zza.zzm().zzd.zzb(j2);
