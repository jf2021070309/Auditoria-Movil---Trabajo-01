...
public final class ColorUtils {
    ...
    public static void XYZToLAB(double x, double y, double z, double[] outLab) {
        ...
        double x2 = pivotXyzComponent(x / 95.047d);
        double y2 = pivotXyzComponent(y / 100.0d);
        double z2 = pivotXyzComponent(z / 108.883d);
        outLab[0] = Math.max((double) PangleAdapterUtils.CPM_DEFLAUT_VALUE, (116.0d * y2) - 16.0d);
    ...
    public static void LABToXYZ(double l, double a, double b, double[] outXyz) {
        ...
        double xr = tmp > 0.008856d ? tmp : ((fx * 116.0d) - 16.0d) / 903.3d;
        double yr = l > 7.9996247999999985d ? Math.pow(fy, 3.0d) : l / 903.3d;
        double tmp2 = Math.pow(fz, 3.0d);
        double zr = tmp2 > 0.008856d ? tmp2 : ((116.0d * fz) - 16.0d) / 903.3d;
        outXyz[0] = 95.047d * xr;
        outXyz[1] = 100.0d * yr;
        outXyz[2] = 108.883d * zr;
    }
