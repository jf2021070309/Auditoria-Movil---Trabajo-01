    ...
    void applyTransformation(float interpolatedTime, Ring ring, boolean lastFrame) {
        ...
        if (this.mFinishing) {
            ...
            if (interpolatedTime >= 0.5f) {
                ...
                scaledTime = endTrim;
                startTrim = endTrim - (((1.0f - MATERIAL_INTERPOLATOR.getInterpolation(scaledTime2)) * 0.79f) + 0.01f);
            } else {
                float scaledTime3 = interpolatedTime / 0.5f;
                startTrim = ring.getStartingStartTrim();
                scaledTime = (MATERIAL_INTERPOLATOR.getInterpolation(scaledTime3) * 0.79f) + 0.01f + startTrim;
            }
            float rotation = (0.20999998f * interpolatedTime) + startingRotation;
            float groupRotation = (this.mRotationCount + interpolatedTime) * 216.0f;
            ring.setStartTrim(startTrim);
