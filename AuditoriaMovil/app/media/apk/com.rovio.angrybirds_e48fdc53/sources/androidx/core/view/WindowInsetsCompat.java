        ...
        protected Insets getInsetsForType(int type, boolean ignoreVisibility) {
            ...
            if (type == 1) {
                if (ignoreVisibility) {
                    ...
                    return Insets.of(0, Math.max(rootStable2.top, getSystemWindowInsets().top), 0, 0);
                }
                Insets rootStable3 = getSystemWindowInsets();
                return Insets.of(0, rootStable3.top, 0, 0);
            }
            if (type == 2) {
                if (ignoreVisibility) {
                    Insets rootStable4 = getRootStableInsets();
                    Insets stable = getStableInsets();
                    return Insets.of(Math.max(rootStable4.left, stable.left), 0, Math.max(rootStable4.right, stable.right), Math.max(rootStable4.bottom, stable.bottom));
                }
                Insets systemWindow = getSystemWindowInsets();
                WindowInsetsCompat windowInsetsCompat = this.mRootWindowInsets;
                rootStable = windowInsetsCompat != null ? windowInsetsCompat.getStableInsets() : null;
                ...
                rootStable = insetsArr != null ? insetsArr[Type.indexOf(8)] : null;
                Insets overriddenInsets = rootStable;
                if (overriddenInsets != null) {
                    return overriddenInsets;
                }
                Insets systemWindow2 = getSystemWindowInsets();
                Insets rootStable5 = getRootStableInsets();
                if (systemWindow2.bottom > rootStable5.bottom) {
                    return Insets.of(0, 0, 0, systemWindow2.bottom);
                }
                Insets insets = this.mRootViewVisibleInsets;
                if (insets != null && !insets.equals(Insets.NONE) && this.mRootViewVisibleInsets.bottom > rootStable5.bottom) {
