...
final class Base64 {
    ...
    private static String encode(byte[] in, byte[] map) {
        ...
        for (int i = 0; i < end; i += 3) {
            int index2 = index + 1;
            out[index] = map[(in[i] & Constants.UNKNOWN) >> 2];
            int index3 = index2 + 1;
            out[index2] = map[((in[i] & 3) << 4) | ((in[i + 1] & Constants.UNKNOWN) >> 4)];
            int index4 = index3 + 1;
            out[index3] = map[((in[i + 1] & cv.m) << 2) | ((in[i + 2] & Constants.UNKNOWN) >> 6)];
            index = index4 + 1;
        ...
        if (i3 == 1) {
            int index5 = index + 1;
            out[index] = map[(in[end] & Constants.UNKNOWN) >> 2];
            int index6 = index5 + 1;
            ...
            out[index] = map[(in[end] & Constants.UNKNOWN) >> 2];
            int index9 = index8 + 1;
            out[index8] = map[((in[end] & 3) << 4) | ((in[end + 1] & Constants.UNKNOWN) >> 4)];
            int index10 = index9 + 1;
            out[index9] = map[(in[end + 1] & cv.m) << 2];
            int i5 = index10 + 1;
            out[index10] = 61;
        }
        try {
