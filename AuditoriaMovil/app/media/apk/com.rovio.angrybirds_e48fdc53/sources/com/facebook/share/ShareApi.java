...
public final class ShareApi {
        ...
        try {
            for (SharePhoto sharePhoto : sharePhotoContent.getPhotos()) {
                try {
                    ...
                    if (bitmap != null) {
                        arrayList.add(GraphRequest.newUploadPhotoRequest(currentAccessToken, getGraphPath("photos"), bitmap, caption, sharePhotoCommonParameters, callback));
                    } else if (imageUrl != null) {
                        arrayList.add(GraphRequest.newUploadPhotoRequest(currentAccessToken, getGraphPath("photos"), imageUrl, caption, sharePhotoCommonParameters, callback));
                    }
                } catch (JSONException e) {
                    ShareInternalUtility.invokeCallbackWithException(facebookCallback, e);
                    return;
    ...
    private void shareLinkContent(ShareLinkContent shareLinkContent, final FacebookCallback<Sharer.Result> facebookCallback) {
        ...
        new GraphRequest(AccessToken.getCurrentAccessToken(), getGraphPath(GMAdConstant.RIT_TYPE_FEED), bundle, HttpMethod.POST, callback).executeAsync();
    }
