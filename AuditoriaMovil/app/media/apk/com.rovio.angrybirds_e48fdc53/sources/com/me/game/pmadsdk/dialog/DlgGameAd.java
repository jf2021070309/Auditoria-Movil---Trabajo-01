...
public class DlgGameAd extends BaseAlertDialog<DlgGameAdLayout> implements OnDialogCloseListener {
    ...
    /* JADX INFO: Access modifiers changed from: protected */
    @Override // com.me.game.pmadsdk.dialog.BaseAlertDialog
    public void initViews(DlgGameAdLayout convertView) {
        convertView.setCloseListener(new View.OnClickListener() { // from class: com.me.game.pmadsdk.dialog.DlgGameAd.1
        ...
        convertView.setClickAdListener(new View.OnClickListener() { // from class: com.me.game.pmadsdk.dialog.DlgGameAd.2
            @Override // android.view.View.OnClickListener
            public void onClick(View v) {
        ...
        String imageUrl = this.mGameAdBean.verticalImageUrl;
        if (isEmpty(imageUrl)) {
            imageUrl = this.mGameAdBean.horizontalImageUrl;
        }
        DisplayMetrics metrics = this.mContext.getResources().getDisplayMetrics();
        if (metrics.widthPixels > metrics.heightPixels) {
            String horizontalImageUr = this.mGameAdBean.horizontalImageUrl;
            if (!isEmpty(horizontalImageUr)) {
                imageUrl = horizontalImageUr;
            }
        }
        if (!isEmpty(imageUrl)) {
            LoggerUtils.e("hhhhh", "imageUrl = " + imageUrl);
            convertView.showAd(imageUrl, this.mGameAdBean.showLogic);
        }
