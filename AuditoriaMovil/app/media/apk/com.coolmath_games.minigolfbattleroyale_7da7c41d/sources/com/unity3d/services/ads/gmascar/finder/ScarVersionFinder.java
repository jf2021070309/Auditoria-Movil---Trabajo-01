...
public class ScarVersionFinder implements IInitializationStatusListener {
    ...
    public void getVersion() {
        try {
            if (!this._presenceDetector.areGMAClassesPresent()) {
                WebViewApp.getCurrentApp().sendEvent(WebViewEventCategory.INIT_GMA, GMAEvent.VERSION, AdRequest.VERSION);
    ...
    public void findAndSendVersion(boolean z) {
        WebViewApp.getCurrentApp().sendEvent(WebViewEventCategory.INIT_GMA, GMAEvent.VERSION, z ? _mobileAdsBridge.getVersionString() : AdRequest.VERSION);
    }
