...
public class HctSolver {
    ...
    public static int findResultByJ(double d, double d2, double d3) {
        ...
        while (i < 5) {
            ...
            double[] matrixMultiply = MathUtils.matrixMultiply(new double[]{inverseChromaticAdaptation(((288.0d * d11) + ((451.0d * d10) + d12)) / 1403.0d), inverseChromaticAdaptation(((d12 - (891.0d * d10)) - (261.0d * d11)) / 1403.0d), inverseChromaticAdaptation(((d12 - (d10 * 220.0d)) - (d11 * 6300.0d)) / 1403.0d)}, LINRGB_FROM_SCALED_DISCOUNT);
            ...
            double d16 = (dArr[2] * d15) + (dArr[1] * d14) + (dArr[0] * d13);
            if (d16 <= 0.0d) {
                return 0;
            }
            if (i != 4) {
    ...
    public static double inverseChromaticAdaptation(double d) {
        ...
        return Math.pow(Math.max(0.0d, (27.13d * abs) / (400.0d - abs)), 2.380952380952381d) * MathUtils.signum(d);
    }
    ...
    public static double[] nthVertex(double d, int i) {
        ...
        if (i < 4) {
            ...
            return isBounded(d7) ? new double[]{d7, d2, d6} : new double[]{-1.0d, -1.0d, -1.0d};
        } else if (i < 8) {
            double d8 = ((d - (d3 * d6)) - (d5 * d2)) / d4;
            return isBounded(d8) ? new double[]{d6, d8, d2} : new double[]{-1.0d, -1.0d, -1.0d};
        } else {
            double d9 = ((d - (d3 * d2)) - (d4 * d6)) / d5;
            return isBounded(d9) ? new double[]{d2, d6, d9} : new double[]{-1.0d, -1.0d, -1.0d};
        }
    }
