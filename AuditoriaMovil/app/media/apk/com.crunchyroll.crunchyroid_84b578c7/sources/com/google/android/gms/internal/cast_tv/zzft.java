package com.google.android.gms.internal.cast_tv;

import com.google.firebase.analytics.FirebaseAnalytics;
/* JADX INFO: Access modifiers changed from: package-private */
/* compiled from: com.google.android.gms:play-services-cast-tv@@21.0.0 */
/* loaded from: classes3.dex */
public final class zzft extends zzfu {
    final transient int zza;
    final transient int zzb;
    final /* synthetic */ zzfu zzc;

    public zzft(zzfu zzfuVar, int i, int i2) {
        this.zzc = zzfuVar;
        this.zza = i;
        this.zzb = i2;
    }

    @Override // java.util.List
    public final Object get(int i) {
        zzfo.zza(i, this.zzb, FirebaseAnalytics.Param.INDEX);
        return this.zzc.get(i + this.zza);
    }

    @Override // java.util.AbstractCollection, java.util.Collection, java.util.List
    public final int size() {
        return this.zzb;
    }

    @Override // com.google.android.gms.internal.cast_tv.zzfr
    public final int zzb() {
        return this.zzc.zzc() + this.zza + this.zzb;
    }

    @Override // com.google.android.gms.internal.cast_tv.zzfr
    public final int zzc() {
        return this.zzc.zzc() + this.zza;
    }

    @Override // com.google.android.gms.internal.cast_tv.zzfr
    public final Object[] zze() {
        return this.zzc.zze();
    }

    @Override // com.google.android.gms.internal.cast_tv.zzfu, java.util.List
    /* renamed from: zzf */
    public final zzfu subList(int i, int i2) {
        zzfo.zzc(i, i2, this.zzb);
        zzfu zzfuVar = this.zzc;
        int i3 = this.zza;
        return zzfuVar.subList(i + i3, i2 + i3);
    }
}
