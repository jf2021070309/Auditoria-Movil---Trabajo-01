...
public final class Cam16 {
    ...
    private static Cam16 fromJchInViewingConditions(double d, double d2, double d3, ViewingConditions viewingConditions) {
        ...
        return new Cam16(d3, d2, d, aw, flRoot, Math.sqrt((c * (d2 / Math.sqrt(d4))) / (viewingConditions.getAw() + 4.0d)) * 50.0d, d5, Math.cos(radians) * log1p, Math.sin(radians) * log1p);
    }
    ...
    public static Cam16 fromXyzInViewingConditions(double d, double d2, double d3, ViewingConditions viewingConditions) {
        ...
        double pow4 = Math.pow((viewingConditions.getNbb() * d16) / viewingConditions.getAw(), viewingConditions.getC() * viewingConditions.getZ()) * 100.0d;
        ...
        double pow5 = Math.pow((hypot * (viewingConditions.getNcb() * nc)) / (d15 + 0.305d), 0.9d) * Math.pow(1.64d - Math.pow(0.29d, viewingConditions.getN()), 0.73d);
        ...
        return new Cam16(d17, sqrt, pow4, flRoot, flRoot2, sqrt2, d19, Math.cos(radians) * log1p, Math.sin(radians) * log1p);
    }
