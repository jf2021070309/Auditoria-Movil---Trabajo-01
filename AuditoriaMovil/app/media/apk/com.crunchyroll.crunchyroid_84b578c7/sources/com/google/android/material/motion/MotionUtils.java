...
public class MotionUtils {
    ...
    private static TimeInterpolator getLegacyThemeInterpolator(String str) {
        if (isLegacyEasingType(str, EASING_TYPE_CUBIC_BEZIER)) {
            ...
            if (split.length == 4) {
                return a.b(getLegacyControlPoint(split, 0), getLegacyControlPoint(split, 1), getLegacyControlPoint(split, 2), getLegacyControlPoint(split, 3));
            }
            throw new IllegalArgumentException("Motion easing theme attribute must have 4 control points if using bezier curve format; instead got: " + split.length);
        } else if (isLegacyEasingType(str, EASING_TYPE_PATH)) {
            return a.c(h.d(getLegacyEasingContent(str, EASING_TYPE_PATH)));
        } else {
            throw new IllegalArgumentException(com.amazon.aps.iva.oa.a.a("Invalid motion easing type: ", str));
        }
    }
