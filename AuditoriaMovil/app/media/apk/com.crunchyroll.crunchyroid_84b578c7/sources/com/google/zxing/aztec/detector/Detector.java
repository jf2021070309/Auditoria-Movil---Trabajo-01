...
public final class Detector {
    ...
    private static CorrectedParameter getCorrectedParameterData(long j, boolean z) throws NotFoundException {
        ...
        try {
            int decodeWithECCount = new ReedSolomonDecoder(GenericGF.AZTEC_PARAM).decodeWithECCount(iArr, i3);
            int i5 = 0;
            for (int i6 = 0; i6 < i2; i6++) {
                i5 = (i5 << 4) + iArr[i6];
            }
            return new CorrectedParameter(i5, decodeWithECCount);
    ...
    public AztecDetectorResult detect(boolean z) throws NotFoundException {
        ...
        int i = this.shift;
        return new AztecDetectorResult(sampleGrid(bitMatrix, bullsEyeCorners[i % 4], bullsEyeCorners[(i + 1) % 4], bullsEyeCorners[(i + 2) % 4], bullsEyeCorners[(i + 3) % 4]), getMatrixCornerPoints(bullsEyeCorners), this.compact, this.nbDataBlocks, this.nbLayers, extractParameters);
    }
}
