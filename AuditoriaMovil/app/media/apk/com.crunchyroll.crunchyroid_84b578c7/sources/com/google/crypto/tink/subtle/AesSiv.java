...
public final class AesSiv implements DeterministicAead {
    ...
    public AesSiv(byte[] bArr) throws GeneralSecurityException {
        if (FIPS.isCompatible()) {
            ...
            throw new InvalidKeyException(e.f(new StringBuilder("invalid key size: "), bArr.length, " bytes; key must have 64 bytes"));
        }
    ...
    private byte[] s2v(byte[]... bArr) throws GeneralSecurityException {
        ...
        if (bArr3.length >= 16) {
            xor = Bytes.xorEnd(bArr3, compute);
        } else {
            xor = Bytes.xor(AesUtil.cmacPad(bArr3), AesUtil.dbl(compute));
        }
