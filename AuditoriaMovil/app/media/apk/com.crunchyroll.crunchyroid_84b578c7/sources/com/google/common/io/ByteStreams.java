...
public final class ByteStreams {
    ...
    public static void skipFully(InputStream inputStream, long j) throws IOException {
        ...
        sb.append(skipUpTo);
        sb.append(" bytes; ");
        throw new EOFException(b.b(sb, j, " bytes expected"));
    }
    ...
    public static long skipUpTo(InputStream inputStream, long j) throws IOException {
        ...
        while (j2 < j) {
            ...
            if (skipSafely == 0) {
                int min = (int) Math.min(j3, (long) MediaStatus.COMMAND_PLAYBACK_RATE);
                if (bArr == null) {
                    bArr = new byte[min];
                }
    ...
    private static byte[] toByteArrayInternal(InputStream inputStream, Queue<byte[]> queue, int i) throws IOException {
        int i2;
        int min = Math.min((int) BUFFER_SIZE, Math.max(128, Integer.highestOneBit(i) * 2));
